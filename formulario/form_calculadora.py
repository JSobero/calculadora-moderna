...
class FormularioCalculadora(tk.Tk):
    ...
    def construir_widget_toggle(self):
        ...
        self.theme_button = tk.Button(self, text="Modo Oscuro \uf186", width=13, font=font_awesome, bd=0, borderwidth=0,
                                      highlightthickness=0, relief=tk.FLAT, command=self.toggle_theme, bg=cons.COLOR_DE_BOTONES_ESPECIALES_LIGHT)
    ...
    def toggle_theme(self):
        if self.dark_theme:
            ...
            self.theme_button.configure(
                text="\uf185 Modo Claro", relief=tk.SUNKEN, bg=cons.COLOR_DE_BOTONES_ESPECIALES_LIGHT)
        else:
            ...
            self.theme_button.configure(
                text="\uf185 Modo Oscuro", relief=tk.SUNKEN, bg=cons.COLOR_DE_BOTONES_ESPECIALES_DARK)
    ...
    def construir_widget(self):
        ...
        for button in buttons:
            ...
            if button in ['=', '*', '/', '-', '+', 'C', '<', '%']:
                color_fondo = cons.COLOR_DE_BOTONES_ESPECIALES_DARK
            ...
            if button == '=':
                tk.Button(self, text=button, width=11, height=2, command=lambda b=button: self.on_button_click(b),
                      bg=color_fondo, fg=cons.COLOR_DE_TEXTO_DARK, relief=tk.FLAT, font=button_font, padx=5, pady=5, bd=0, borderwidth=0, highlightthickness=0, overrelief='flat').grid(row=row_val, column=col_val,columnspan=2, pady=5)
                col_val += 1
            else:
                tk.Button(self, text=button, width=5, height=2, command=lambda b=button: self.on_button_click(b),
                      bg=color_fondo, fg=cons.COLOR_DE_TEXTO_DARK, relief=tk.FLAT, font=button_font, padx=5, pady=5, bd=0, borderwidth=0, highlightthickness=0, overrelief='flat').grid(row=row_val, column=col_val, pady=5)
                col_val += 1
